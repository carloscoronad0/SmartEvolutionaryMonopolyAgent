...
class SmartAgent(Agent):
    def __init__(self, agent_id: int, regular_memory_replay, trading_memory_replay):
        ...
        self.regular_actor = DdqnActor(REGULAR_INPUT_NUM, REGULAR_OUTPUT_NUM, 0.003, 0.80, 0.02, 0.05, regular_memory_replay, "regular", 64)
        self.trading_actor = DdqnActor(TRADING_INPUT_NUM, TRADING_OUTPUT_NUM, 0.001, 0.01, 0.02, 0.5, trading_memory_replay, "trading", 64)
    ...
    def make_trade_offer(self, trade_binary_representation: np.ndarray) -> MAs.TradeActionStructure:
        ...
        if player_selection[0] < len(self.order_dic):
            target_player_id = self.order_dic[player_selection[0]]
        else:
            target_player_id = -1

        trade_offer = MAs.TradeActionStructure(MAs.ActionType.MakeTradeOffer, offer, asked, money_offer, money_ask,
            target_player_id)
    ...
    def improve_property(self, streets_binary: np.ndarray) -> MAs.PropertyActionStructure:
        ...
        indicated_idxs = np.where(indicated_streets == 1)
        return MAs.PropertyActionStructure(MAs.ActionType.ImproveProperty, indicated_idxs, MAs.PropertyType.Street)

    def sell_house_hotel(self, streets_binary: np.ndarray) -> MAs.PropertyActionStructure:
        indicated_streets = np.round(streets_binary)
        indicated_idxs = np.where(indicated_streets == 1)
        return MAs.PropertyActionStructure(MAs.ActionType.SellHouseOrHotel, indicated_idxs, MAs.PropertyType.Street)
    ...
    def mortgage_properties(self, mortgage_binary: np.ndarray) -> MAs.PropertyActionStructure:
        ...
        indicated_idxs = np.where(indicated_mortgage == 1)
        return MAs.PropertyActionStructure(MAs.ActionType.MortgageProperty, indicated_idxs, MAs.PropertyType.Property)

    def free_mortgage(self, free_binary: np.ndarray) -> MAs.PropertyActionStructure:
        indicated_free = np.round(free_binary)
        indicated_idxs = np.where(indicated_free == 1)
        return MAs.PropertyActionStructure(MAs.ActionType.FreeMortgage, indicated_idxs, MAs.PropertyType.Property)

    def conclude_actions(self, conclude_binary) -> MAs.BinaryActionStructure:
        conclude = np.round(conclude_binary) == 1
        return MAs.BinaryActionStructure(MAs.ActionType.ConcludeActions, conclude)

    def use_get_out_of_jail_card(self, use_card_binary) -> MAs.BinaryActionStructure:
        use_card = np.round(use_card_binary) == 1
        return MAs.BinaryActionStructure(MAs.ActionType.UseGetOutOfJailCard, use_card)

    def pay_jail_fine(self, pay_fine_binary) -> MAs.BinaryActionStructure:
        pay_fine = np.round(pay_fine_binary) == 1
        return MAs.BinaryActionStructure(MAs.ActionType.PayJailFine, pay_fine)

    def buy_property(self, buy_binary) -> MAs.BinaryActionStructure:
        buy = np.round(buy_binary) == 1
        return MAs.BinaryActionStructure(MAs.ActionType.PayJailFine, buy)
    ...
    def accept_trade_offer(self, accept_binary) -> MAs.BinaryActionStructure:
        ...
        return MAs.BinaryActionStructure(MAs.ActionType.AcceptTradeOffer, accept)
    ...
    def continue_in_auction(self, continue_binary) -> MAs.AuctionActionStructure:
        ...
        return MAs.AuctionActionStructure(MAs.ActionType.ContinueInAuction, cont, money_to_bid)
    ...
    def form_regular_state(self, state: MSs.RegularMonopolyState) -> np.ndarray:
        ...
        if difference > 0:
            for _ in range(difference):
                # Padding
                total_info.append([0 for _ in range(5)])
        ...
        for prop in state.propertiesInGame:
            temp_owner = [0 for i in range(INITIAL_NUMBER_OF_PLAYERS)]
            if prop.owner != None:
    ...
    def take_regular_decisions(self, state: MSs.RegularMonopolyState, action_list: List[MAs.ActionType]) -> List[MAs.ActionStructure]:
        ...
        for action in action_list:
            if action == MAs.ActionType.MakeTradeOffer:
                ...
                decision_structures.append(mortgage_section)
    ...
    def take_trading_decision(self, state: MSs.OfferActionMonopolyState, action_list: List[MAs.ActionType]) -> List[MAs.ActionStructure]:
